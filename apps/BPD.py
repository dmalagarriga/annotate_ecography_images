...
image_directory_BPD = '../images/BPD/'
list_of_images_BPD = [os.path.basename(x) for x in glob.glob('{}*.jpg'.format(image_directory_BPD))]
filelist =  [os.path.join(image_directory_BPD,image) for image in np.sort(list_of_images_BPD)]
...
image_annotation_card = dbc.Card(
    id="imagebox",
    children=[
        dbc.CardHeader(html.H2("Annotation area for BPD")),
        dbc.CardBody(
            [
                dcc.Graph(
                    id="app-1-graph",
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Store(id="app-1-graph-copy", data=fig),
                            dcc.Store(
                                id="app-1-annotations-store",
                                data=dict(
                                    **{
                                        filename: {"shapes": []}
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Dropdown(
                                id="app-1-annotation-type-dropdown",
                                options=[
            ...
            [
                html.Div(
                    [
                        ...
                        dbc.Button(
                            "Download annotations", id="app-1-download-button", outline=True,
                        ),
                        html.Div(id="app-1-dummy", style={"display": "none"}),
                        dbc.Tooltip(
                            "You can download the annotated data in a .json format by clicking this button",
                            target="app-1-download-button",
                        ),
                    ],
                )
            ]
...
layout = html.Div(
    ...
    html.Div(id='page-BPD-content'),
    ]
)


@app.callback(
    [Output("app-1-annotations-table", "data"), Output("app-1-image_files", "data")],
    [
        Input("previous", "n_clicks"),
        Input("next", "n_clicks"),
        Input("app-1-graph", "relayoutData"),
    ],
    [
        State("app-1-annotations-table", "data"),
        State("app-1-image_files", "data"),
        State("app-1-annotations-store", "data"),
        State("app-1-annotation-type-dropdown", "value"),
    ],
...
def modify_table_entries(
    ...
    if cbcontext == "app-1-graph.relayoutData":
        ...
        if annotations_table_data is None:
            ...
            with open('results_annotation/BPD/data_BPD_tst_%s.json' %image_files_data["current"], 'w') as outfile:
                json.dump(annotations_table_data, outfile,indent=6,ensure_ascii=True)
            outfile.close()
...
@app.callback(
    [Output("app-1-graph", "figure"), Output("app-1-annotations-store", "data"),],
    [Input("app-1-annotations-table", "data"), Input("app-1-annotation-type-dropdown", "value")],
    [State("app-1-image_files", "data"), State("app-1-annotations-store", "data")],
    prevent_initial_call=True,
)
...
@app.callback(
    Output("app-1-modal", "is_open"),
    [Input("app-1-howto-open", "n_clicks"), Input("app-1-howto-close", "n_clicks")],
    [State("app-1-modal", "is_open")],
    prevent_initial_call=True,
)
...
# set the download url to the contents of the app-1-annotations-store (so they can be
...
""",
    Output("app-1-download", "href"),
    [Input("app-1-annotations-store", "data")],
)
...
{
    let download_a=document.getElementById("app-1-download");
    download_a.click();
    return '';
}
""",
    Output("app-1-dummy", "children"),
    [Input("app-1-download-button", "n_clicks")],
)
...
@app.callback(
    Output("app-1-navbar-collapse", "is_open"),
    [Input("app-1-navbar-toggler", "n_clicks")],
    [State("app-1-navbar-collapse", "is_open")],
)
