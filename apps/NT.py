...
image_directory_NT = '../images/NT/'
list_of_images_NT = [os.path.basename(x) for x in glob.glob('{}*.jpg'.format(image_directory_NT))]
filelist =  [os.path.join(image_directory_NT,image) for image in np.sort(list_of_images_NT)]
...
image_annotation_card = dbc.Card(
    id="imagebox",
    children=[
        dbc.CardHeader(html.H2("Annotation area for NT")),
        dbc.CardBody(
            [
                dcc.Graph(
                    id="app-3-graph",
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Store(id="app-3-graph-copy", data=fig),
                            dcc.Store(
                                id="app-3-annotations-store",
                                data=dict(
                                    **{
                                        filename: {"shapes": []}
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Dropdown(
                                id="app-3-annotation-type-dropdown",
                                options=[
            ...
            [
                html.Div(
                    [
                        ...
                        dbc.Button(
                            "Download annotations", id="app-3-download-button", outline=True,
                        ),
                        html.Div(id="app-3-dummy", style={"display": "none"}),
                        dbc.Tooltip(
                            "You can download the annotated data in a .json format by clicking this button",
                            target="app-3-download-button",
                        ),
                    ],
                )
            ]
    ...
    dbc.Container(
        [
            ...
            dbc.Row(
                dbc.Col(
                    [
                        dbc.NavbarToggler(id="app-3-navbar-toggler"),
                        dbc.Collapse(
                            dbc.Nav(
...
layout = html.Div(
    ...
    html.Div(id='page-NT-content'),
    ]
)


@app.callback(
    [Output("app-3-annotations-table", "data"), Output("app-3-image_files", "data")],
    [
        Input("previous", "n_clicks"),
        Input("next", "n_clicks"),
        Input("app-3-graph", "relayoutData"),
    ],
    [
        State("app-3-annotations-table", "data"),
        State("app-3-image_files", "data"),
        State("app-3-annotations-store", "data"),
        State("app-3-annotation-type-dropdown", "value"),
    ],
...
def modify_table_entries(
    ...
    if cbcontext == "app-3-graph.relayoutData":
        ...
        if annotations_table_data is None:
            ...
            with open('results_annotation/NT/data_NT_tst_%s.txt' %image_files_data["current"], 'w') as outfile:
...
@app.callback(
    [Output("app-3-graph", "figure"), Output("app-3-annotations-store", "data"),],
    [Input("app-3-annotations-table", "data"), Input("app-3-annotation-type-dropdown", "value")],
    [State("app-3-image_files", "data"), State("app-3-annotations-store", "data")],
    prevent_initial_call=True,
)
...
@app.callback(
    Output("app-3-modal", "is_open"),
    [Input("app-3-howto-open", "n_clicks"), Input("app-3-howto-close", "n_clicks")],
    [State("app-3-modal", "is_open")],
    prevent_initial_call=True,
)
...
# set the download url to the contents of the app-3-annotations-store (so they can be
...
""",
    Output("app-3-download", "href"),
    [Input("app-3-annotations-store", "data")],
)
...
{
    let download_a=document.getElementById("app-3-download");
    download_a.click();
    return '';
}
""",
    Output("app-3-dummy", "children"),
    [Input("app-3-download-button", "n_clicks")],
)
...
@app.callback(
    Output("app-3-navbar-collapse", "is_open"),
    [Input("app-3-navbar-toggler", "n_clicks")],
    [State("app-3-navbar-collapse", "is_open")],
)
