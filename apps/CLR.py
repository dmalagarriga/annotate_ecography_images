...
image_directory_CLR = '../images/CLR/'
list_of_images_CLR = [os.path.basename(x) for x in glob.glob('{}*.jpg'.format(image_directory_CLR))]
filelist =  [os.path.join(image_directory_CLR,image) for image in np.sort(list_of_images_CLR)]
...
image_annotation_card = dbc.Card(
    id="imagebox",
    children=[
        dbc.CardHeader(html.H2("Annotation area for CLR")),
        dbc.CardBody(
            [
                dcc.Graph(
                    id="app-2-graph",
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Store(id="app-2-graph-copy", data=fig),
                            dcc.Store(
                                id="app-2-annotations-store",
                                data=dict(
                                    **{
                                        filename: {"shapes": []}
                ...
                dbc.Row(
                    dbc.Col(
                        [
                            ...
                            dcc.Dropdown(
                                id="app-2-annotation-type-dropdown",
                                options=[
            ...
            [
                html.Div(
                    [
                        ...
                        dbc.Button(
                            "Download annotations", id="app-2-download-button", outline=True,
                        ),
                        html.Div(id="app-2-dummy", style={"display": "none"}),
                        dbc.Tooltip(
                            "You can download the annotated data in a .json format by clicking this button",
                            target="app-2-download-button",
                        ),
                    ],
                )
            ]
    ...
    dbc.Container(
        [
            ...
            dbc.Row(
                dbc.Col(
                    [
                        dbc.NavbarToggler(id="app-2-navbar-toggler"),
                        dbc.Collapse(
                            dbc.Nav(
...
layout = html.Div(
    ...
    html.Div(id='page-CLR-content'),
    ]
)


@app.callback(
    [Output("app-2-annotations-table", "data"), Output("app-2-image_files", "data")],
    [
        Input("previous", "n_clicks"),
        Input("next", "n_clicks"),
        Input("app-2-graph", "relayoutData"),
    ],
    [
        State("app-2-annotations-table", "data"),
        State("app-2-image_files", "data"),
        State("app-2-annotations-store", "data"),
        State("app-2-annotation-type-dropdown", "value"),
    ],
...
def modify_table_entries(
    ...
    if cbcontext == "app-2-graph.relayoutData":
        ...
        if annotations_table_data is None:
            ...
            with open('results_annotation/CLR/data_CLR_tst_%s.txt' %image_files_data["current"], 'w') as outfile:
                json.dump(annotations_table_data, outfile)
...
@app.callback(
    [Output("app-2-graph", "figure"), Output("app-2-annotations-store", "data"),],
    [Input("app-2-annotations-table", "data"), Input("app-2-annotation-type-dropdown", "value")],
    [State("app-2-image_files", "data"), State("app-2-annotations-store", "data")],
    prevent_initial_call=True,
)
...
@app.callback(
    Output("app-2-modal", "is_open"),
    [Input("app-2-howto-open", "n_clicks"), Input("app-2-howto-close", "n_clicks")],
    [State("app-2-modal", "is_open")],
    prevent_initial_call=True,
)
...
# set the download url to the contents of the app-2-annotations-store (so they can be
...
""",
    Output("app-2-download", "href"),
    [Input("app-2-annotations-store", "data")],
)
...
{
    let download_a=document.getElementById("app-2-download");
    download_a.click();
    return '';
}
""",
    Output("app-2-dummy", "children"),
    [Input("app-2-download-button", "n_clicks")],
)
...
@app.callback(
    Output("app-2-navbar-collapse", "is_open"),
    [Input("app-2-navbar-toggler", "n_clicks")],
    [State("app-2-navbar-collapse", "is_open")],
)
